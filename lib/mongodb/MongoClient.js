...
class MongoClient {
  ...
  /**
   * 
   * @param {String} schema Get documents in the schema
   */
  async size(schema){
    if(!schema) throw new TypeError(`Schema is a required argument.`);
    let Data = db.getData("client", ":client:");
    let SCHEMA = schema ? schema : "DEFAULT"; 
    return await Data.Schemas[SCHEMA].find().length;
  }
}
/*
